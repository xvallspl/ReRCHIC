	...
	methods = list( 
			...
			getMaximumSimilarity = function(level = levels){
			 ...
			 getJoinedClasses = function(at.level=levels, primitivesOnly = FALSE){
				...
				if(!primitivesOnly) n <- nPrimitiveClasses+at.level-1 else n <- nPrimitiveClasses
				return(unique(array(which(joinMatrix[1:n, 1:n] <= at.level , arr.ind = T))))
			 },
